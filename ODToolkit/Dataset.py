...
class DatasetMaker:
    ...
    def visualize(self, homograph=False, homo=None):
        n = len(self.img)
    ...
    def _copy_paste_augmentation(self,
                                 train_img: List[str], 
                                 train_lbl: List[str], 
                                 K:int, 
                                 savepath_img:str, 
    ...
    def create_aug_dataset(self,
        ...
        failed = 0
        invalid = 0
        for idx, i in enumerate(tqdm(self.aug_train_img)):
            _, fname = os.path.split(i)
            ...
            while cnt<repeat:
                try:
                    status = self._copy_paste_augmentation(self.aug_train_img, self.aug_train_lbl, 
                                                           K=idx, 
                    ...
                    if status == -1: # no label or invalid for augmentation
                        invalid += 1
                        shutil.copy2(self.aug_train_img[idx], savepath_img + '{}.jpg'.format(name))
                        shutil.copy2(self.aug_train_lbl[idx], savepath_lbl + '{}.txt'.format(name))
                        break
                    else:
                        cnt+=1
